function Install-LabWindowsAdminCenter {
    ...
    if ($machines) {
        ...
        $certDownload = Get-LabInternetFile -Uri (Get-LabConfigurationItem -Name WacMsIntermediateCaCert) -Path "$(Get-LabSourcesLocation -Local)/SoftwarePackages" -FileName MicCodSigPCA2011_2011-07-08.crt -PassThru -NoDisplay
        $wacDownload = Get-LabInternetFile -Uri (Get-LabConfigurationItem -Name WacDownloadUrl) -Path "$(Get-LabSourcesLocation -Local)/SoftwarePackages" -FileName WAC.exe -PassThru -NoDisplay
        ...
        Invoke-LabCommand -ComputerName $machines -NoDisplay -ScriptBlock {
            Add-Certificate2 -Path C:\MicCodSigPCA2011_2011-07-08.crt -Store CA -Location CERT_SYSTEM_STORE_LOCAL_MACHINE
            Add-Certificate2 -Path C:\MicCodSigPCA2011_2011-07-08.crt -Store CA -Location CERT_SYSTEM_STORE_CURRENT_USER
        }
    }
    ...
    $jobs = foreach ($labMachine in $machines) {
        ...
        Install-LabSoftwarePackage -LocalPath C:\WAC.exe -CommandLine '/SILENT /LOG=C:\DeployDebug\WACSetup.log' -ComputerName $labMachine -UseShellExecute -AsJob -PassThru -NoDisplay
    }
