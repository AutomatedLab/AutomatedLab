    ...
    $jobs = foreach ($labMachine in $machines) {
        ...
        if ($labMachine.SshPrivateKeyPath) {
            Invoke-LabCommand -ComputerName $labMachine -ScriptBlock {
                "Start-Process -Wait -FilePath C:\WAC.exe -ArgumentList '/SILENT /LOG=C:\DeployDebug\WACSetup.log'; exit 0" | Set-Content C:\AL\DeployWac.ps1
                powershell.exe -File C:\AL\DeployWac.ps1
            } -AsJob -PassThru -NoDisplay
        }
        else {
            Install-LabSoftwarePackage -LocalPath C:\WAC.exe -CommandLine '/SILENT /LOG=C:\DeployDebug\WACSetup.log' -ComputerName $labMachine -PassThru -NoDisplay -AsJob
        }
    }
