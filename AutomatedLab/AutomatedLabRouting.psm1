...
#region Install-LabVpnGateway
function Install-LabVpnGateway
{
    # .ExternalHelp AutomatedLab.Help.xml
    [cmdletBinding()]
    param (
    ...
    Write-ScreenInfo -Message 'Configuring VPN Gateway role...'
    ...
    # Routing: Was passiert mit bestehndem lab router? 
    ...
    foreach ($machine in $machines)
    {
        ...
        $scriptBlock = {
            ...
            $networkAdapter = Get-NetAdapter | Where-Object MacAddress -eq $ExternalMac
			$networkAdapter | Rename-NetAdapter -NewName 'External'
			...
			ConvertTo-MaskLength $vnet.AddressSpace.Netmask.AddressAsString
