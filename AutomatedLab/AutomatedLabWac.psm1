function Install-LabWindowsAdminCenter
{
    ...
    <#
    WAC Role Properties
    Credential = When connecting e.g. to existing WAC, provide different Credential
    Port
    EnableDevMode
    #>
...
function Add-LabWacManagedNode
{
    ...
    foreach ($machine in $machines)
    {
        ...
        $wacCredential = if ($role.Properties.ContainsKey('Credential'))
        {
            $role.Properties['Credential']
        }
        else
        {
        ...
        Write-ScreenInfo -Message "Adding $((Get-LabVm | Where-Object -Property Name -ne $ComputerName).Count) hosts to the admin center for user $($wacCredential.UserName)"
        $apiEndpoint = "https://$($wachostname):$Port/api/connections"

        $bodyHash = foreach ($vm in (Get-LabVm | Where-Object -Property Name -ne $machine.Name))
        {
            @{
        ...
        try
        {
            ...
            Write-ScreenInfo -Message "Successfully added all lab machines as connections for $($wacCredential.UserName)"
        }
        catch
        {
